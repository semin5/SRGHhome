...
export function Galaxy(){
 ...
 useFrame(({pointer,clock},delta)=>{if(!galaxy.current)return;const portrait=aspect<1;const t=Math.min(delta*.9,1);const rotationX=(portrait?-1.01:-1.17)+pointer.y*.09;const rotationY=(portrait?-.07:-.23)+pointer.x*.13;const rotationZ=(portrait?.48:-.56)+clock.elapsedTime*.018;galaxy.current.rotation.x+=(rotationX-galaxy.current.rotation.x)*t;galaxy.current.rotation.y+=(rotationY-galaxy.current.rotation.y)*t;galaxy.current.rotation.z+=(rotationZ-galaxy.current.rotation.z)*t;galaxy.current.position.x+=(0-galaxy.current.position.x)*t;galaxy.current.position.y+=(0-galaxy.current.position.y)*t;galaxy.current.position.z+=(0-galaxy.current.position.z)*t});
