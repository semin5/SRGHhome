...
export function Galaxy(){
 ...
 useFrame(({pointer,clock},delta)=>{if(!galaxy.current)return;const portrait=aspect<1;const t=Math.min(delta*1.7,1);const rotationX=(portrait?.48:.62)+pointer.y*.15;const rotationY=(portrait?-.18:.3)+pointer.x*.22;const rotationZ=(portrait?-.76:.58)+Math.sin(clock.elapsedTime*.16)*.055+pointer.x*.06;galaxy.current.rotation.x+=(rotationX-galaxy.current.rotation.x)*t;galaxy.current.rotation.y+=(rotationY-galaxy.current.rotation.y)*t;galaxy.current.rotation.z+=(rotationZ-galaxy.current.rotation.z)*t;galaxy.current.position.x+=(pointer.x*.55-galaxy.current.position.x)*t;galaxy.current.position.y+=(-pointer.y*.35+Math.sin(clock.elapsedTime*.22)*.08-galaxy.current.position.y)*t});
 return <points ref={galaxy} position={[0,0,0]} rotation={[aspect<1?.48:.62,aspect<1?-.18:.3,aspect<1?-.76:.58]} scale={aspect<1?1.72:Math.max(1.5,aspect/1.38)} frustumCulled={false}>
