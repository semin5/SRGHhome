...
function Planet({service,index,active,reduced,onChoose}:{service:Service;index:number;active:number;reduced:boolean;onChoose:(i:number)=>void}){
 ...
 target.current.set(focused?(mobile?0:3.3):(mobile?service.position[0]*.48:service.position[0]*.43+3.4),focused?(mobile?-1.7:0):(mobile?service.position[1]-1.8:service.position[1]*.95-.15),focused?1:service.position[2]);
 if(!reduced)target.current.y+=Math.sin(clock.elapsedTime*.35+index)*.07;
 group.current.position.lerp(target.current,reduced?1:Math.min(delta*3.5,1));const s=hidden?.001:focused?3.6:hover?1.1:1;scale.current.setScalar(s);group.current.scale.lerp(scale.current,reduced?1:Math.min(delta*4,1));
 });
 return <group ref={group} position={[service.position[0],service.position[1]-1.8,service.position[2]]}>
 <mesh onPointerOver={()=>setHover(true)} onPointerOut={()=>setHover(false)} onClick={e=>{e.stopPropagation();onChoose(index+1)}}><sphereGeometry args={[service.radius,64,64]}/><meshPhysicalMaterial color={service.color} roughness={.58} metalness={.22} clearcoat={.18} clearcoatRoughness={.7}/></mesh>
...
export default function Home(){
 ...
 <section id="sec01" data-index={0} ref={el=>{sections.current[0]=el}} className="full-section home-section"><div className="hero-copy"><p className="eyebrow">YOUR WORK, IN ORBIT</p><h1>Our work.<br/><span>Connected.</span></h1><p className="hero-description">우리의 일과 사람, 가능성이<br/>하나의 우주에서 연결됩니다.</p></div></section>
